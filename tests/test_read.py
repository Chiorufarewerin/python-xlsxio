...
class TestReadXlsx(TestCase):
    reader: xlsxio.XlsxioReader

    def setUp(self):
        self.reader = xlsxio.XlsxioReader(XLSX_TEST_FILE_BASE)
    ...
    def test_data_without_types(self):
        for sheet_name in self.reader.get_sheet_names():
            with self.reader.get_sheet(sheet_name, flags=xlsxio.XLSXIOREAD_SKIP_NONE) as sheet:
                data = sheet.read_data()
            self.assertListEqual(data, TEST_READ_DATA_WITHOUT_TYPES[sheet_name])

    def test_data_types(self):
        for sheet_name in self.reader.get_sheet_names():
            with self.reader.get_sheet(sheet_name, flags=xlsxio.XLSXIOREAD_SKIP_NONE, types=TYPES) as sheet:
                data = sheet.read_data()
            self.assertListEqual(data, TEST_READ_DATA_TYPES[sheet_name])

    def test_data_bytes(self):
        for sheet_name in self.reader.get_sheet_names():
            with self.reader.get_sheet(sheet_name, flags=xlsxio.XLSXIOREAD_SKIP_NONE, default_type=bytes) as sheet:
                data = sheet.read_data()
    ...
    def test_flag_skip_empty_rows(self):
        with self.reader.get_sheet('test_empty', flags=xlsxio.XLSXIOREAD_SKIP_EMPTY_ROWS) as sheet:
            data = sheet.read_data()
    ...
    def test_flag_skip_empty_cells(self):
        with self.reader.get_sheet('test_empty', flags=xlsxio.XLSXIOREAD_SKIP_EMPTY_CELLS) as sheet:
            data = sheet.read_data()
    ...
    def test_flag_skip_all_empty(self):
        with self.reader.get_sheet('test_empty', flags=xlsxio.XLSXIOREAD_SKIP_ALL_EMPTY) as sheet:
            data = sheet.read_data()
